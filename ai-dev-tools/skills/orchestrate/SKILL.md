...
**Runtime resolution of `<RESUME-BREADCRUMB>`:** emit exactly one line — `/orchestrate` if the hint file's `mode:` field is `standard` (or no mode field, or the hint file has not yet been written), or `/orchestrate --strict` if `mode:` is `strict` (or the current invocation was called with `--strict`). Never emit the literal `<RESUME-BREADCRUMB>` placeholder and never emit both variants — the breadcrumb must be a single concrete command as the literal last line of the RED output.
...
- **Case B (0 criticals, 0 highs — success):** 3-option multi-line breadcrumb. Option 1 is `/clear → /orchestrate` (Standard) or `/clear → /orchestrate --strict` (Strict) — advance to Step 8, recommended, phase boundary. Options 2 and 3 are two randomly-selected quality gates from the **Quality-Gate Pool** subsection below, rendered as `/clear → /<gate-name>`, re-randomized on every emission. Render all three options as bare commands stacked one per line per the Multi-Line Breadcrumb Format subsection. In strict mode, Option 1 gets the `--strict` token per Strict Mode Breadcrumbs; quality-gate options do not (quality-gate skills are orchestrate-agnostic).
...
**Step 7 — Fix Findings:** review-code-summary.md Critical or High >0, commits match feature. Apply fixes, re-run `/review-code`. Loop 6-7 until clean. After re-invoking `/review-code`, emit the breadcrumb per the Strict Mode Breadcrumbs and Exit Output Format subsections (Standard: `/orchestrate (/review-code <N> --against <spec_path> --max-iterations 3)`; Strict: `/orchestrate --strict (/review-code <N> --against <spec_path> --max-iterations 3)`) as the literal last line — matches the Step 7 row in the Step-to-command mapping table. Edge: NOT `/respond-to-review` -- that is for doc reviews only.
