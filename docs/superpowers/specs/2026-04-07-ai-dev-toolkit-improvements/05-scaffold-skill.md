...
The `node-fastify-react` stack uses 22 placeholders across the three template layers (`root/`, `package/`, `technology/`). The full list lives in `references/placeholder-resolution.md`. SKILL.md documents only the **resolution sources** at a high level:
...
- **Layer 3 (`technology/`)** → in v1, **merges** into the root files written in Layer 1. Specifically: appends technology/CLAUDE.md content to root/CLAUDE.md as a new section, and merges technology/.claude/settings.json into the just-written root/.claude/settings.json (deep-merge of `permissions`, `hooks`, `network` keys; technology values win on conflict).
...
1. **Update `pnpm-workspace.yaml`:** read existing file, parse YAML, add `packages/<name>` to the `packages:` list (preserving order and comments via a careful YAML round-trip), write back. If the entry already exists → skip silently.
...
13. **Read the created `SKILL.md`** and confirm it documents all 22 placeholders by reference (pointing to `placeholder-resolution.md`), and that the per-mode flow tables match the spec.
...
- **Three-layer template composition** (root → package → technology) with `{{PLACEHOLDER}}` substitution. 22 placeholders total, resolved via auto-derivation, interactive prompts, or `--config <path>.yaml`. Reference table for placeholders lives in `references/placeholder-resolution.md`.
...
- **Layer 3 (`technology/`) merges into Layer 1 root files** rather than writing separately — appends to root CLAUDE.md, deep-merges into root settings.json (technology values win on conflict).
