...
The `node-fastify-react` stack uses ~20 placeholders across the three template layers (`root/`, `package/`, `technology/`) — the full authoritative list is generated by scanning the templates during implementation and lives in `references/placeholder-resolution.md`. SKILL.md documents only the **resolution sources** at a high level:
