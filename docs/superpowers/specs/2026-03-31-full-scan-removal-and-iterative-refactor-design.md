...
- First layer single-unit spec at `docs/superpowers/specs/YYYY-MM-DD-layer-<layer-name>-design.md` with **Status: Draft**. Contains: layer boundary definition, files belonging to this layer, dependency violations to resolve (imports that cross layer boundaries downward), provider interfaces to introduce, expected import rewrites. The hint written after --next-unit (step: 2) drives the routing to Step 2 on the next invocation; no additional spec-scanning logic is added to orchestrate.
